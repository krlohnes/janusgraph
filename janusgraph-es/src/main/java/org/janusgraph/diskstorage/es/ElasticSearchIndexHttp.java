...
public class ElasticSearchIndexHttp extends AbstractElasticSearchIndex {

    private static final Logger log = LoggerFactory.getLogger(ElasticSearchIndexHttp.class);
    ...
    public ElasticSearchIndexHttp(Configuration config) {
    ...
    private JestClient createClient(Configuration config) {
        ...
        HttpClientConfig clientConfig = new HttpClientConfig.Builder(serverUris)
            ...
            .discoveryEnabled(config.get(CLIENT_SNIFF)
     ...
     */
    private void checkForOrCreateIndex(Configuration config) throws BackendException {
        ...
        if (!result.isSucceeded()) {
            if (result.getResponseCode() == 404) {
                /*
                 * Use aliases for ease of operational support; actual index name is
                 * suffixed with the index creation time. This aids in index
                 * resharding as graph grows
                 */
                Long now = System.currentTimeMillis();
                String localIndexName = this.indexName + ":" + now.toString();
                ImmutableSettings.Builder sb = ImmutableSettings.builder()
                    .put("number_of_shards", this.configuration.get(SHARD_COUNT))
                    .put("number_of_replicas", this.configuration.get(REPLICA_COUNT));

                CreateIndex ci = new CreateIndex.Builder(localIndexName)
                    .settings(sb.build().getAsMap())
    ...
    public void register(String store, String key, KeyInformation information, BaseTransaction tx) throws BackendException {
        ...
        if (!result.isSucceeded()) {
            throw new TemporaryBackendException(formatException("PutMapping", result));
        }
    }
    ...
    //TODO No changes between the two
